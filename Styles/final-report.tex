...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccccccc}
    \toprule
    Model & Val loss $\downarrow$ & Val CER $\downarrow$ & Val DER & Val IER & Val SER & Test loss $\downarrow$ & Test CER $\downarrow$ & Test DER & Test IER & Test SER \\
    \midrule
...
Table~\ref{tab:architecture-results} now mixes measured values from the current
training logs with `?` placeholders where the corresponding metric was not yet
recorded in the draft. In the final version, replace the remaining placeholders
and state which model achieved the best validation CER and which model achieved
the best held out test CER. If a model performs inconsistently across
validation and test, discuss that explicitly rather than hiding it.
...
We next evaluated how reducing the number of EMG channels affects decoding performance. Table~\ref{tab:channel-subset-comparison} summarizes the results summarizes the results of training the hybrid CNN+GRU model with several different channel subsets. 
...
\item {\bf Claims}
    ...
    \item[] Justification: Yes, our claims focus on performance of different model architectures, and the rest of the papers justify the rest of the claims.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Limitations}
    ...
    \item[] Answer: \answerYes{} % Replace by \answerYes{}, \answerNo{}, or \answerNA{}.
    \item[] Justification: Yes, we discuss the limitations of our approach in the paper.
    \item[] Guidelines:
    \begin{itemize}
